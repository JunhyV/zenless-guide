...
const AgentView = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const fromElementColor = {
      fire: "from-rose-800",
      physical: "from-yellow-800",
      electric: "from-blue-800",
      ice: "from-cyan-800",
      ether: "from-fuchsia-800",
    };
  ...
  return (
    <div className="lg:w-1/2 min-h-full relative overflow-hidden">
      <img
        ...
        alt={nickname}
        className={`min-h-full min-w-full object-cover object-top ${adjust} ${screenHeight2}`}
      />

      <div
        className={`border-4 ${borderColor} bg-gradient-to-r ${fromColor} ${toColor} text-white absolute flex gap-5 p-1 md:p-2 rounded-xl ${screenHeight}`}
      >
        <img
        ...
        <div className="w-fit my-auto">
          <p className="text-xl md:text-4xl font-medium">{full_name}</p>
          <p className="text-lg md:text-2xl">{faction}</p>
        </div>
      </div>

      <div className="absolute top-2 left-2 md:top-5 md:left-5 flex flex-col gap-2">
        <div
          className={`border-4 ${borderColor} bg-gradient-to-r ${fromColor} ${toColor} p-1 w-40 md:w-52 rounded-br-full flex gap-5 items-center`}
        >
          <img
          ...
          />
          <p className="text-lg md:text-2xl text-white capitalize font-medium">
            {rol}
          </p>
        </div>
        <div
          className={`border-4 ${borderColor} bg-gradient-to-r ${fromColor} ${toColor} p-1 w-36 md:w-44 rounded-br-full flex gap-5 items-center`}
        >
          <img
